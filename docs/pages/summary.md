...
- **Experiments** — the compiler is a way to test compilation techniques in a real project without too much complexity. It contains artifacts from experiments such as *symtable compression*, *perceptron inline*, *peephole DSL*, and the *AST static analyzer*. See the main README for further details.
...
- **Function overloading** — the compiler supports function overloading with restrictions.
...
- **Annotations** — the language supports annotations as a way to extend the otherwise restricted grammar.
- **Assembly generation** — the compiler generates assembly for further compilation.
