...
A simple language that behaves like a subset of C language with some things from Rust is a perfect solution of my problem. This new language can be treated as a language that somewhere is more advanced than C language, and somewhere is more restricted than C language. Justification of such restrictions (like intentionally removed structures, enums and classes) is an interesting task as well. </br>
P.S.: *Also, according to my explicit bias to assembly and C languages, the new language is intended to remain as low-level as possible, but some features can be added in future with strings (build-in concat, comparison and etc).*
