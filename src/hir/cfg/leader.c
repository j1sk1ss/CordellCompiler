...
int HIR_CFG_mark_leaders(cfg_ctx_t* ctx) {
    foreach (cfg_func_t* fb, &ctx->funcs) {
        ...
        set_add(&fb->leaders, h);                            /* Rule 1 - First instruction in a function          */
        while (h) {
            if (h->op == HIR_MKLB) set_add(&fb->leaders, h); /* Rule 2 - Target of a JMP instruction              */
            else if (
                HIR_is_jmp(h->op) &&                         /* Rule 3 - Next instruction after a JMP instruction */
                h->next
