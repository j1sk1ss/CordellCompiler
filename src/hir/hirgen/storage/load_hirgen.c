...
hir_subject_t* HIR_generate_load(ast_node_t* node, hir_ctx_t* ctx, sym_table_t* smt) {
    ...
    switch (node->t->t_type) {
        ...
        case UNKNOWN_FLOAT_NUMERIC_TOKEN: res = HIR_SUBJ_NUMBER(HIR_F64NUMBER, node->t->body); break;
        case CALL_ADDR_TOKEN: {
            res = HIR_SUBJ_TMPVAR(HIR_TMPVARI0, VRTB_add_info(NULL, TMP_I0_TYPE_TOKEN, NO_SYMBOL_ID, EMPTY_BASIC_FLAGS, &smt->v));
            res->ptr = 1;
