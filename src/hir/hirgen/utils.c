...
hir_subject_t* HIR_add_to_subject(hir_subject_t* src, sym_table_t* smt, long add, hir_ctx_t* ctx) {
    hir_subject_t* add_subj = HIR_SUBJ_TMPVAR(src->t, VRTB_ADD_TMP_NF(src->t, smt));
...
hir_subject_t* HIR_gdref_subject(hir_subject_t* src, sym_table_t* smt, hir_ctx_t* ctx, int set_ptr) {
    hir_subject_t* dref_subj = HIR_SUBJ_TMPVAR(src->t, VRTB_ADD_TMP_NF(src->t, smt));
    if (set_ptr == FIELD_NO_CHANGE) dref_subj->ptr = src->ptr - 1;
    else                            dref_subj->ptr = set_ptr;
