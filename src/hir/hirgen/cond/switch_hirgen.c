...
static int _generate_case_binary_jump(
    ...
    HIR_BLOCK3(ctx, HIR_iLWR, is_lwr, cond, HIR_generate_implconv(ctx, cond->t, HIR_SUBJ_CONST(val), smt));
    ...
    HIR_BLOCK3(ctx, HIR_iLRG, is_grt, cond, HIR_generate_implconv(ctx, cond->t, HIR_SUBJ_CONST(val), smt));
