...
static int _convert_lirblock_to_assembly(lir_block_t* b, func_info_t* fi, sym_table_t* smt, FILE* output) {
    ...
    switch (b->op) {
        ...
        case LIR_FCLL:
        case LIR_ECLL: EMIT_COMMAND("call %s", i386_gnu_nasm_format_lir_subject(b->farg, smt, NO_FLAG)); break;
        case LIR_STRT:
        case LIR_FDCL: {
        ...
        case LIR_NOT:        EMIT_COMMAND("neg %s", i386_gnu_nasm_format_lir_subject(b->farg, smt, NO_FLAG));                                                                    break;
