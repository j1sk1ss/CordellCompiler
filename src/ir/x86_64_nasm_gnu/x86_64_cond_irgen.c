...
int IR_generate_if_block(ast_node_t* node, ir_gen_t* g, ir_ctx_t* ctx) {
    ...
    IR_BLOCK2(ctx, iCMP, IR_SUBJ_REG(RAX, 8), IR_SUBJ_CNST(0));
    if (rbranch) IR_BLOCK1(ctx, JE, IR_SUBJ_STR(8, "_f%d", current_label));
    else IR_BLOCK1(ctx, JE, IR_SUBJ_STR(8, "_e%d", current_label));

    g->blockgen(lbranch->child, g, ctx);
    IR_BLOCK1(ctx, JMP, IR_SUBJ_STR(8, "_e%d", current_label));

    if (rbranch) {
        IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_f%d", current_label));
        g->blockgen(rbranch->child, g, ctx);
    }

    IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_e%d", current_label));
    return 1;
}
...
int IR_generate_while_block(ast_node_t* node, ir_gen_t* g, ir_ctx_t* ctx) {
    ...
    IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_w%d", current_label));
    g->elemegen(cond, g, ctx);
    IR_BLOCK2(ctx, iCMP, IR_SUBJ_REG(RAX, 8), IR_SUBJ_CNST(0));
    IR_BLOCK1(ctx, JE, IR_SUBJ_STR(8, "_e%d", current_label));
    
    g->blockgen(lbranch->child, g, ctx);
    IR_BLOCK1(ctx, JMP, IR_SUBJ_STR(8, "_w%d", current_label));
...
static int _generate_case_binary_jump(int* values, int left, int right, int label_id, int default_scope, ir_ctx_t* ctx) {
    if (left > right) {
        if (default_scope) IR_BLOCK1(ctx, JMP, IR_SUBJ_STR(8, "_d%d", label_id));
        else IR_BLOCK1(ctx, JMP, IR_SUBJ_STR(8, "_e%d", label_id));
        return 0;
    }
    ...
    IR_BLOCK2(ctx, iCMP, IR_SUBJ_REG(RAX, 8), IR_SUBJ_CNST(val));
    IR_BLOCK1(ctx, JL, IR_SUBJ_STR(8, "_cl%d%d", val, label_id));
    IR_BLOCK1(ctx, JG, IR_SUBJ_STR(8, "_cr%d%d", val, label_id));
    IR_BLOCK1(ctx, JMP, IR_SUBJ_STR(8, "_c%d%d", val, label_id));

    IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_cl%d%d", val, label_id));
    _generate_case_binary_jump(values, left, mid - 1, label_id, default_scope, ctx);

    IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_cr%d%d", val, label_id));
...
int IR_generate_switch_block(ast_node_t* node, ir_gen_t* g, ir_ctx_t* ctx) {
    ...
    IR_BLOCK1(ctx, MKLB, IR_SUBJ_STR(8, "_ec%d", current_label));
