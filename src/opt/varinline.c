...
static int _find_usage(
    ast_node_t* root, char* varname, short s_id, int* status, int offset
) {
    if (!root) return 0;
    for (ast_node_t* t = root->child; t; t = t->sibling) {
        if (offset-- > 0) continue;
        if (!t->token || t->token->t_type == SCOPE_TOKEN) {
            _find_usage(t, varname, s_id, status, 0);
            continue;
        }
        ...
        switch (t->token->t_type) {
            ...
            case SYSCALL_TOKEN:
            case DEFAULT_TOKEN:
            case ARRAY_TYPE_TOKEN: _find_usage(t, varname, s_id, status, 0);                          continue;
            case FUNC_TOKEN:       _find_usage(t->child->sibling->sibling, varname, s_id, status, 0); continue;
            default: break;
        }
        ...
        if (VRS_isdecl(t->token) || VRS_isoperand(t->token)) {
            ...
            _find_usage(t, varname, s_id, status, VRS_isdecl(t->token));
            continue;
        }
...
static int _find_decl(ast_node_t* root, ast_node_t* entry, int* change) {
    ...
    for (ast_node_t* t = root->child; t; t = t->sibling) {
        ...
        if (VRS_isdecl(t->token) && VRS_one_slot(t->token)) { /* Can be optimized */
            ...
            int is_used = 0;
            _find_usage(entry, name_node->token->value, name_node->info.s_id, &is_used, 0);
            if (is_used) {                
