...
static int _find_assign(
    ...
    for (ast_node_t* t = root->child; t; t = t->sibling) {
        ...
        if ((VRS_isdecl(t->token) && VRS_one_slot(t->token)) || t->token->t_type == ASSIGN_TOKEN) {
            return _find_assign(t->child->sibling, varname, s_id, status, local);
        }

        if (!str_strncmp(varname, t->child->token->value, TOKEN_MAX_SIZE) && t->child->info.s_id == s_id) {
            *status = 1;
            return 1;
        }
    }
...
static int _change_decl(
    ast_node_t* root, char* varname, short s_id, int value, int local, int offset
) {
    if (!root) return 0;
    int index = 0;
    for (ast_node_t* t = root->child; t; t = t->sibling) {
        ...
        if (!t->token || t->token->t_type == SCOPE_TOKEN) {
            _change_decl(t, varname, s_id, value, local, 0);
            continue;
        }
        
        switch (t->token->t_type) {
            case CASE_TOKEN:       _change_decl(t, varname, s_id, value, local, 0); break;
            case INT_TYPE_TOKEN:
            case LONG_TYPE_TOKEN:
            case CHAR_TYPE_TOKEN: 
            case SHORT_TYPE_TOKEN: _change_decl(t, varname, s_id, value, local, 1); continue;
            case IF_TOKEN:
            case EXIT_TOKEN:
            case CALL_TOKEN:
            case PLUS_TOKEN:
            ...
            case ARRAY_TYPE_TOKEN: _change_decl(t, varname, s_id, value, local, 0); continue;
            case FUNC_TOKEN: 
                if (!local) {
                    _change_decl(t->child->sibling->sibling, varname, s_id, value, local, 0);
                }
            continue;
            default: break;
        }
...
static int _find_decl(ast_node_t* root, ast_node_t* entry, int* change) {
    ...
    for (ast_node_t* t = root->child; t; t = t->sibling) {
        ...
        if (VRS_isdecl(t->token) && VRS_one_slot(t->token)) { /* Can be optimized */
            ...
            if (!VRS_intext(t->token)) _find_assign(entry, name_node->token->value, name_node->info.s_id, &is_used, 0);
            else _find_assign(root, name_node->token->value, name_node->info.s_id, &is_used, 1);
            ...
            if (is_used) {
                ...
                if (!VRS_intext(t->token)) _change_decl(entry, name_node->token->value, name_node->info.s_id, value, 0, 0);
                else _change_decl(root, name_node->token->value, name_node->info.s_id, value, 1, 0);
                *change = 1;
            }
        }
    }
