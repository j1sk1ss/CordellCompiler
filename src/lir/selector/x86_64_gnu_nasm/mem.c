                        ...
                        if (ARTB_get_info(lh->farg->storage.var.v_id, &ai, &smt->a)) {
                            if (ai.vla) { // TODO: VLA
                                ...
                                foreach (lir_subject_t* elem, &lh->targ->storage.list.h) {
                                    ...
                                    LIR_insert_block_before(
                                        LIR_create_block(LIR_aMOV, LIR_SUBJ_OFF(RBP, arr_off - elem_info.slot_off, elem_info.slot_size), elem, NULL), lh
                                    );
...
static lir_block_t* _create_push_arg_mov(lir_subject_t* dst, lir_subject_t* src, sym_table_t* smt) {
    ...
    if (
        src->size == 4 && !x86_64_gnu_nasm_is_sign_type(src, smt) &&
        ...
        movop = x86_64_gnu_nasm_get_proper_mov(dst, src, smt, LIR_iMOV);
    }
        ...
        if (lh->farg) {
            switch (lh->op) {
                case LIR_PUSH: {
                    ...
                    lir_subject_t* tmp = x86_64_gnu_nasm_create_tmp(R15, lh->farg, smt, 8);
                ...
                case LIR_POP: {
                    ...
                    lir_subject_t* tmp = x86_64_gnu_nasm_create_tmp(R15, dst, smt, 8);
