...
static inline int _same_full_register(lir_subject_t* a, lir_subject_t* b) {
    ...
    return LIR_format_register(a->storage.reg.reg, 4) == LIR_format_register(b->storage.reg.reg, 4);
}
                ...
                switch (lh->op) {
                    ...
                    case LIR_ECLL: {
                        ...
                        for (int i = 0; i < regs_count; i++) {
                            lir_block_t* push = LIR_create_block(LIR_PUSH, LIR_SUBJ_REG(regs[i], 4), NULL, NULL);
                            LIR_insert_block_after(push, push_pos);
                            push_pos = push;
                        }

                        for (int i = regs_count - 1; i >= 0; i--) {
                            LIR_insert_block_before(LIR_create_block(LIR_POP, LIR_SUBJ_REG(regs[i], 4), NULL, NULL), post);
                        }
