...
To accomplish such a task you will need to modify the `build_targinfo.c` file. It isn't hard, but you will need to obtain / inquier neccesiry information about your acrhitecture. In the nutshell, you will need to find information such as `commutative` (flag), `issue_cost` (heuristic value), `latency` (value),  `reads_memory` (flag), `sets_flags` (flag), `throughput` (value), `uses_flags` (flag) and `writes_memory` (flag). </br>
