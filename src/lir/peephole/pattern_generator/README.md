...
1. PTRN supports multiple lines as a single pattern. This allows us to build a complex logic:
```ptrn
mov areg_1, areg_2
mov areg_2, areg_1
->
...
1. PTRN also supports simple math and basic if-else statements. This makes the next code a valid construction:
```ptrn
...
1. `Delete` is pretty easy to use. Just put this command below the `->` identifier, and it will remove all lines that are satisfies to the choosen patter:
```ptrn
add obj, const 0
->
