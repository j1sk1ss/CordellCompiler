...
\begin{abstract}
\par This paper presents Cordell Programming Language (CPL), a compact C-like systems language that keeps C's direct access to memory, layout, and machine interfaces while experimenting with a smaller grammar and selected conveniences from newer languages. CPL provides primitive values, pointers, arrays, containers with methods, unions, generic functions, overloads, entry-point and section control, system calls, and inline assembly without attempting source compatibility with full C. The article describes the language design, compiler pipeline, target backends, static-analysis architecture, and OS-facing use cases, then evaluates the prototype backend with reproducible x86\_64 and i386 microbenchmarks against C compiler baselines.
\end{abstract}
...
\par CPL is not a C subset. A faithful C-like subset would either require a substantially more complex parser and semantic model, or would create misleading expectations that the rest of C should also be supported. The design instead favors a compact parser-friendly grammar, explicit syntax for functions and pointer-like operations, and compiler-supported low-level constructs.
...
\par The contributions of this article are:
\begin{itemize}[leftmargin=*]
  ...
  \item preliminary performance evidence across x86-64 and i386 Linux backends;
  \item an SSA/SMT diagnostic architecture whose demonstrated capabilities and unmeasured properties are separated from implementation claims.
\end{itemize}
...
\par CPL minimizes dependencies on a host standard library. The compiler supports x86-64 Mach-O, x86-64 Linux, and i386 Linux assembly paths. The Mach-O path is the oldest backend, while the Linux paths are important for freestanding and OS-oriented examples.
