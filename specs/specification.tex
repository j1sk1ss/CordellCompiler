...
\par These annotations are not intended to make CPL a high-level language. Their role is to expose system-level control without expanding the core grammar.
...
\par Function inlining exists, but it is not stable enough to be used as an unconditional compiler-quality claim.
...
\par Structures, classes, enums, and related user-defined aggregate types remain outside the current core by design. They may be studied separately, but this report treats their absence as part of CPL's deliberate restriction rather than a missing feature in the current language model.
