...
\begin{abstract}
\par Cordell Programming Language (CPL) is a deliberately restricted systems programming language and compiler infrastructure designed for low-level experimentation. Its syntax is close to C and assembly in intent, but it avoids implementing a C subset. The language exposes explicit control over pointers, arrays, strings, entry points, system calls, inline assembly, and system-oriented annotations, while intentionally avoiding structures, classes, enums, garbage collection, memory safety guarantees, and production-language ecosystem assumptions. The compiler is organized as a multi-stage pipeline from preprocessing and tokenization to AST construction, semantic checking, IR generation, SSA construction, high-level and low-level optimization, instruction selection, register allocation, assembly emission, and linking. This report documents the current language core, compiler pipeline, implemented static diagnostics, optimization passes, testing infrastructure, target status, preliminary microbenchmarks, limitations, and related work. The goal is not to present CPL as a verified or production-ready replacement for C or Rust, but as a compact experimental platform for studying compiler architecture, systems-language design, optimization, and diagnostics.
\end{abstract}

\section{Introduction}
\par Systems programming languages expose machine-level details while attempting to keep programs tractable. Mature languages such as C~\cite{}, C++~\cite{}, Rust~\cite{}, and Zig~\cite{} provide large ecosystems, but their full syntactic and semantic complexity makes them expensive targets for compiler experiments. Conversely, many teaching compilers (such as "Writing A Compiler in Go"~\cite{}) are intentionally small but do not expose enough low-level mechanisms to study system-oriented code generation, platform-specific interfaces, and optimization under realistic constraints.
\par CPL is not a C subset. A faithful C-like subset would either require a substantially more complex parser and semantic model, or would create misleading expectations that the rest of C should also be supported. The current design instead favors an LL(1)-oriented grammar~\cite{}, explicit syntax for functions and pointer-like operations, and compiler-supported low-level constructs.

\par The contribution of this report is a consolidated technical description of CPL v3.5~\cite{}:
\begin{itemize}[leftmargin=*]
  \item a concise language overview and design rationale;
  \item the current multi-stage compiler pipeline;
  \item the high IR, high-low IR, low-low IR, and assembly form of a small example;
...
After lowering to High Low IR (HLIR), the program is represented as a basic-block sequence.

\begin{lstlisting}[caption={High Low IR form.}]
BB1: start
%2 = $1 as i32;
%0 = %2;
%3 = $2 as i32;
...
A later Low Low IR (LLIR) form contains target-dependent register choices.
