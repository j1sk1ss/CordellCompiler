...
\par A practical use case for CPL is small freestanding routines used by an operating-system kernel. The i386 backend was added to support this style of code: globally visible routines can be emitted as NASM-compatible 32-bit assembly, while inline assembly keeps exact control over privileged instructions and interrupt-frame layout.

\par Listing~\ref{lst:i386-switch-user} is adapted from a real kernel helper. The function prepares segment registers, pushes an i386 interrupt-return frame, and transfers execution to ring 3 with \texttt{iretd}. The \texttt{naked} annotation is essential here. It suppresses the usual generated function frame, so the compiler does not emit a prologue such as \texttt{push ebp; mov ebp, esp}. As a result, the first stack argument remains at \texttt{[esp + 4]} and the second at \texttt{[esp + 8]}; in a non-naked function that used a saved frame pointer, equivalent argument access would normally be based on the post-prologue frame layout.

\begin{lstlisting}[language=CPL,caption={i386 ring-transition helper using a naked CPL function.},label={lst:i386-switch-user}]
@[nosection]
@[naked]
...
\par The corresponding generated i386 assembly loads the CPL parameters directly from the caller's stack before expanding the inline assembly placeholders:

\begin{lstlisting}[caption={Generated i386 assembly for the ring-transition helper.}]
...
\par Because the function is emitted as a global cdecl-compatible symbol, it can also be called directly from C kernel code. A matching C prototype is:

\begin{lstlisting}[language=C,caption={C prototype for calling the CPL i386 helper.}]
void __attribute__((cdecl)) i386_switch2user(void* entry, void* stack);
\end{lstlisting}

\par Using CPL instead of raw assembler keeps these routines within the compiler pipeline: declarations and calls can still participate in static diagnostics, symbol checking, and target-specific regression tests. The CPL source also documents the function interface and placeholder binding more directly than a standalone assembly file, which improves readability without removing access to exact machine instructions.

\par This example illustrates why CPL exposes annotations and inline assembly instead of hiding every backend detail behind a high-level ABI. Kernel code often needs a function body that is exactly the machine sequence expected by the processor. A smaller helper from the same i386 test set flushes the task-state segment by emitting \texttt{ltr 0x28}; unlike the ring-transition helper, it does not need \texttt{naked}, so a normal \texttt{ret} remains appropriate after the inline assembly.
