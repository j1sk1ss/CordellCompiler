...
\title{Minimal extention for C-language as a compact system programming language}
\author{
...
\begin{abstract}
\par The paper investigates whether a compact systems language can expose the binary layout and machine-interface controls required by freestanding software while retaining a conventional optimizing pipeline and SSA-based static diagnostics. Cordell Programming Language (CPL) provides primitive values, pointers, arrays, aggregates and unions, entry-point and section control, system calls, and inline assembly without implementing the full semantics of C. Paper covers general implementation of the language, its grammar, its pipeline and overall possible use-cases. The developed language intentionally stays close to C language which helps in demonstration of possible extention of existed languages with modern syntax approaches such as generic functions, structure method containers, overloads, etc.
\end{abstract}
